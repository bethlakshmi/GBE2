...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for volunteer in orm.Volunteer.objects.all():
            if not volunteer.title or len(volunteer.title) == 0:
                volunteer.title = 'volunteer bid: %s' % volunteer.profile.display_name
                volunteer.save()

    def backwards(self, orm):
        for volunteer in orm.Volunteer.objects.all():
            if volunteer.title == 'volunteer bid: %s' % volunteer.profile.display_name:
                volunteer.title = ''
                volunteer.save()
