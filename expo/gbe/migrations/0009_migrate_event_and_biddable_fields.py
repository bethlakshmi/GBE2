...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        biddables = [orm.Act, orm.Class, orm.Volunteer,
                     orm.Vendor, orm.AdBid, orm.ArtBid,
                     orm.Costume]
        events = [orm.Show, orm.Class, orm.GenericEvent]
        for biddable in biddables:

            for bid in biddable.objects.all():
                bid.b_title = bid.title
                bid.b_description = bid.description
                bid.b_conference = bid.conference
                bid.save()
        for eventtype in events:
            for event in eventtype.objects.all():
                event.e_title = event.title
                event.e_description = event.description
                event.e_conference = event.conference
                event.save()
