...
class Duration(timedelta):
    ...
    def __div__(self, other):
        ...
        duration, if dividing by a duration, return an int (the ratio of the two 
        durations)
        Note that all division operations on Durations are calculated in terms of seconds. This can lead 
        to unexpected results, for example Duration (70) % 60 => 0 (not 10!). 
        '''
        if isinstance(other, int) or isinstance(other, long):
            ...
            raise TypeError ("Unsupported operation: can only divide Duration by int or duration/timedelta")


    def __floordiv__(self, other):
        '''
        Behavior depends on the divisor. If dividing a duration by an int, return a 
        duration, if dividing by a duration (including a timedelta), return an 
        int (the ratio of the two durations)        
        Note that all division operations on Durations are calculated in terms of seconds. This can lead 
        to unexpected results, for example Duration (70) % 60 => 0 (not 10!). 
        '''
        if isinstance(other, int) or isinstance(other, long):
    ...
    def __mod__ (self, other):
        ...
        (if we take 130 minutes mod 60 minutes, we have a proportion of 2 and a remainder of ten 
        minutes: 60 minutes * 2 + 10 minutes = 130 minutes. On the other hand if we take 130 
        minutes mod 2, we get 60 minutes + 10 minutes, since 60 minutes * 2 + 10 minutes = 130 minutes)
        Note that all division operations on Durations are calculated in terms of seconds. This can lead 
        to unexpected results, for example Duration (70) % 60 => 0 (not 10!). 
        '''
        if isinstance(other, int) or isinstance(other, long):
            ...
            return Duration(seconds = long(self.total_seconds()%other.total_seconds()))
        else : 
            raise TypeError ("Unsupported operation: can only take mod of Duration by int and Duration or timedelta")
    ...
    def __divmod__(self, other):
        ...
        try:
            ...
            return TypeError ("Unsupported operation, cannot divide Duration by one of those")
